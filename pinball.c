...
void pinballInitVars(Game* g) {
	...
	pinballCol = getColor(GRAY, 255);
	pinballStroke = getColor(BLACK, 255);
	pinballAlpha = 255;
	pinballJerkPower = -5;
...
void pinball(Game* g) {
	...
	for (int i = 0; i < 2; i++) {
		stroke(getColor(BLACK, 255), paddleRadius * 2);
	...
	if (pinballPos.y > g->y + g->h * 13 / 16) {
		...
		pinballAlpha -= 30;
		pinballCol = getColor(GRAY, pinballAlpha);
		pinballStroke = getColor(BLACK, pinballAlpha);
